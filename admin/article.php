...
<?php
$titre_page = "Article";
$conn = new PDO('mysql:host=localhost;dbname=yourtouch', 'root', '');
include("../inc/functions.php");

if (isset($_GET['id']) and !empty($_GET['id'])) {
   $get_id = htmlspecialchars($_GET['id']);
   $article = $conn->prepare('SELECT * FROM articles INNER JOIN users ON users.idUser=articles.idUser WHERE idArticle = ?');
   $article->execute(array($get_id));
  
   if ($article->rowCount() == 1) {
      $article = $article->fetch();
      $pseudoarticle = $article['pseudo'];
      $titre = $article['titre'];
      $contenu = nl2br($article['contenuArticle']);
      $date = new DateTime($article['dateArticle']);
         $url = "../imgarticle/";
         if($article["cheminImage"] == ""){
            $img = "default.png";
         }
         else {
            $img = $article["cheminImage"];
         }

         $profilArticle = "photo_profil.png";
         if($article['photoProfil'] != ""):
               $profilArticle = $article["photoProfil"];
         endif;
      
      $commentaire = $conn->prepare('SELECT *, pseudo FROM commentaires INNER JOIN users ON commentaires.idUser=users.idUser WHERE commentaires.idArticle = '.$article['idArticle']);
      $commentaire->execute();
       
      $contenuCommentaire = "";
      $dateCom = "";
      $pseudo = "";
      $photoProfil = 'photo_profil.png';
      $result = $conn->prepare('SELECT COUNT(`idCommentaire`) FROM `commentaires` WHERE `idArticle` ='. $article['idArticle']);
      $result->execute();
...
<body>
   <header>
         <nav id="menu">
            <a href="index.php" class="logo logoArticle">
...
<main class="mArticle">
   ...
   <!-- <img src="<?php echo $url.$img ?>" alt="erreur" /> --> 
      <div class="imgarticle" style="background-image: url('<?=$url.$img?>')"></div> 
   </article>
     ...
     <div class="moitierarticle">
     	<div>
         <div class="posteurarticle" >

            <div class="pic" style="background-image: url('<?= $url.$profilArticle ?>')"></div>
         ...
         <div class="commentaireArticleMere">
            <?php while ($com = $commentaire->fetch()):
                     $profilcom = "photo_profil.png";
               if($com['photoProfil'] != ""):
                     $profilcom = $com["photoProfil"];
               endif;
            ?>
            <div class="groupcommentaire">   
               <div class="photoCommentaire" style="background-image: url('<?= $url.$profilcom?>')"></div>
               <div class="commentaireArticle" id="commentaire">
                  <div> <?= $com['pseudo'] ?> <div class="datecommentaire" style="font-size:12px; font-weight: 100;"><?= htmlentities($com['dateCommentaire'], ENT_QUOTES, "UTF-8")?></div></div>
         ...
         <div class="nombrecommentaireArticle">
         <?=$num_rows[0]?> commentaires
         </div>

         <div>
            <form method="post" action="../commentaire.php" class="formArticle">
               <input type="hidden" name="articleID" value="<?= $article['idArticle'];?>" />
               <textarea name="commentaire" cols="33px" rows="2" placeholder="Ajouter un commentaire..."></textarea>
   ...
   <a href="javascript:history.go(-1)" style="color:#013ADF" class="retourenarrier">
      <svg xmlns="http://www.w3.org/2000/svg" width="65" height="65" viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
   </a>
</main>
</body>
